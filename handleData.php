...
<?php
	include 'connectvarsEECS.php'; //info to connect to the database
	//connect to the database
	$conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
	if (!$conn) { //if the connection fails
		die('Could not connect: ' . mysql_error());
	}
	//get data from the html sign up form
	$bio = mysqli_real_escape_string($conn, $_REQUEST['bio']);
	$pic = mysqli_real_escape_string($conn, $_REQUEST['pic']);
	$email = mysqli_real_escape_string($conn, $_REQUEST['email']);
	$mod = mysqli_real_escape_string($conn, $_REQUEST['mod']);
	$userName = mysqli_real_escape_string($conn, $_REQUEST['username']);
	$password = mysqli_real_escape_string($conn, $_REQUEST['password']);
 

	
	$sql = "INSERT INTO Users (Username, Password, Email, Bio, Pic, Moderator) VALUES ('$userName', '$password', '$email', '$bio', '$pic', '$mod')";
	if(mysqli_query($conn, $sql)){ //if the user is entered into the database
		echo "User added successfully.";
	}
	else{ //throw error
		echo "ERROR: Data could not be entered." . mysqli_error($conn);
	}
